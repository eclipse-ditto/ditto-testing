...
/**
 * This matcher requests a PATCH for a resource of Ditto and checks if it worked as expected.
 */
@NotThreadSafe
public final class PatchMatcher extends HttpVerbMatcher<PatchMatcher> {

    private static final String PATCH_MESSAGE_TEMPLATE = "PATCHing <{}> at <{}> with contentType {}: {}";
    private static final String SHORT_MESSAGE_TEMPLATE = "PATCHing <{}> at <{}> with contentType {}";

    private final String contentType;
    @Nullable private final String payload;

    private PatchMatcher(final String path, final String contentType, @Nullable final String payload) {
        super(path);
        this.contentType = contentType;
        this.payload = payload;
    }

    private PatchMatcher(final String path, @Nullable final String jsonString) {
        this(path, ContentType.JSON.toString(), jsonString);
    }

    /**
     * Creates a new {@code PatchMatcher} for the given {@code path}.
     *
     * @param path the path of the resource to put.
     * @throws NullPointerException if {@code path} is {@code null}.
     * @throws IllegalArgumentException if {@code path} is empty.
     */
    public static PatchMatcher newInstance(final String path) {
        return new PatchMatcher(path, null);
    }

    /**
     * Creates a new {@code PatchMatcher} for the given {@code path} and {@code jsonString}.
     *
     * @param path the path of the resource to put.
     * @param jsonString the JSON to put.
     * @throws NullPointerException if any argument is {@code null}.
     */
    public static PatchMatcher newInstance(final String path, final String jsonString) {
        return new PatchMatcher(path, checkNotNull(jsonString, "JSON string"));
    }

    /**
     * Creates a new {@code PatchMatcher} for the given {@code path} and {@code payload}.
     *
     ...
     * @throws IllegalArgumentException if {@code path} is empty.
     */
    public static PatchMatcher newInstance(final String path, final String contentType,
            @Nullable final String payload) {
        checkNotNull(path, "path");
        checkNotNull(contentType, "content-type");
        return new PatchMatcher(path, contentType, payload);
    }
    ...
    protected Response doFire(final String path, final ResponseSpecification responseSpecification,
        ...
        return responseSpecification.when().patch(path);
    }
